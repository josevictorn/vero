...
describe("Fetch Screening Flows Controller (e2e)", () => {
  ...
  test("[GET] /screening-flows", async () => {
    const user = await accountFactory.makePrismaAccount();
    const accessToken = jwt.sign({ sub: user.id.toString() });

    await Promise.all([
      screeningFlowFactory.makePrismaScreeningFlow(),
      screeningFlowFactory.makePrismaScreeningFlow(),
    ]);
    ...
    expect(response.body.screeningFlows).toHaveLength(2);
  });
});
